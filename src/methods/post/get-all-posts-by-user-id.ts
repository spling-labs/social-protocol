...
import { GetAllLikesByPublicKeysQuery, GetUserByUserIdQuery, Order_By, Splinglabs_0_1_0_Decoded_Likes, Splinglabs_0_1_0_Decoded_Post } from '../../gql/graphql'
import { GetAllPostByUserIdQueryDocument } from '../../utils/gql/post'
 ...
 * @category Post
 * 
 * @param {number} userId The user id where the posts should be fetched.
 ...
 */
export default async function getAllPostsByUserId(userId: number, limit: number | null = null, offset: number | null = null, orderBy: Order_By | null = null): Promise<Post[]> {
  try {
    const posts: Post[] = []
    ...
    if (this.graphQLClient !== null) {
      ...
      const tagsAndPosts = await this.graphQLClient.batchRequests([
        ...
        { document: GetAllPostByUserIdQueryDocument, variables: { limit: limit, offset: offset, orderBy: orderBy, userId: userId } }
      ])
      ...
      // Get user to load posts.
      const user: GetUserByUserIdQuery = await this.graphQLClient.request(GetUserByUserIdQueryDocument, { userId: userId })
      ...
      const postsContentPromises: Promise<PostFileDataV2>[] = onChainPosts.map(post => getPostFileDataV2(post.pid, new web3.PublicKey(post.cl_pubkey), userShdwPublicKey))

      // Read user file from shadow drive.
      const userContent: UserFileDataV2 | null = await getUserFileDataV2(userId, userShdwPublicKey)
      if (userContent === null) throw new UserNotFoundError()

      // Read all post files from shadow drive.
      let postsContent: PostFileDataV2[] = await Promise.all(postsContentPromises)
      ...
      let postsText: PostTextFileData[] = await Promise.all(postsContent.map(postContent => {
        if (postContent.text === null) return null
        ...
        try {
          ...
          posts.push({
            ...
            user: {
              ...
              avatar:
                userContent.avatar != null
                  ? `${shadowDriveDomain}${userChain.shdw.toString()}/${userContent.avatar.file}`
                  : null,
            } as PostUser,
