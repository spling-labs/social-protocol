...
export default async function createPost(
  ...
  try {
    ...
    const postTextFile = new File(
      [new Blob([text], { type: "text/plain" })],
      `${index}.txt`
    );

    let fileSizeSummarized = 1000 + postImageFile.size + postTextFile.size; // 1000 bytes will be reserved for the post.json.
    ...
    return Promise.resolve({
      text: text,
      image: `${shadowDriveDomain}${account.publicKey.toString()}/${
        postJson.image
      }`,
    } as Post);
  } catch (error) {
